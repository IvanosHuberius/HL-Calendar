...
Remove-Item -Force "$base\pkg_jwcalendar_v1.8.3.zip" -ErrorAction SilentlyContinue
...
Write-Host "Building pkg_jwcalendar_v1.8.3.zip..."
New-ZipWithForwardSlashes -SourceDir $base -ZipPath "$base\pkg_jwcalendar_v1.8.3.zip" -Include @('pkg_jwcalendar.xml','packages','language')
Write-Host "  OK"

# Verify structure
Write-Host "`nVerifying ZIP structure..."
$zip = [System.IO.Compression.ZipFile]::OpenRead("$base\pkg_jwcalendar_v1.8.3.zip")
$zip.Entries | ForEach-Object { Write-Host "  $($_.FullName)" }
$zip.Dispose()

$fi = Get-Item "$base\pkg_jwcalendar_v1.8.3.zip"
